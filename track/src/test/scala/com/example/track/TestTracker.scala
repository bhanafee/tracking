  ...
  "A Tracker" should {
    ...
    "when sent a waypoint" should {
      "ignore a waypoint with no tags or position" in {
        val test = TestActorRef[Tracker]
        test ! Waypoint("empty", Instant.now(), None, No_Tags)
        assert(test.underlyingActor.waypoints.isEmpty)
      }
      "accept a valid waypoint" in {
        val test = TestActorRef[Tracker]
        test ! Waypoint("id", Instant.now(), Somewhere, No_Tags)
      ...
      "accept multiple waypoints" in {
        val test = TestActorRef[Tracker]
        test ! Waypoint("id1", Instant.now(), Somewhere, No_Tags)
        test ! Waypoint("id2", Instant.now(), Somewhere, No_Tags)
        test ! Waypoint("id3", Instant.now(), Somewhere, No_Tags)
        test ! Waypoint("id4", Instant.now(), Somewhere, No_Tags)
        val result = test.underlyingActor.waypoints
    ...
    "when queried" should {
      "send the most recent by id" in {
        val test = TestActorRef[Tracker]
        test ! Waypoint("id", Instant.now(), Somewhere, No_Tags)
        val future = test ? com.example.track.Tracker.Query("id")
      ...
      "respond empty when there are no matches" in {
        val test = TestActorRef[Tracker]
        test ! Waypoint("id", Instant.now(), Somewhere, No_Tags)
        val future = test ? Tracker.Query("NOT_A_MATCH")
        val result = future.value.get.get.asInstanceOf[Track]
      ...
      "respond empty when there are no waypoints since a timestamp" in {
        val test = TestActorRef[Tracker]
        val now = Instant.now()
        test ! Waypoint("id", now.minusSeconds(100L), Somewhere, No_Tags)
        test ! Waypoint("id", now.minusSeconds(90L), Somewhere, No_Tags)
        test ! Waypoint("id", now.minusSeconds(80L), Somewhere, No_Tags)
        val future = test ? Tracker.Query("id", now.minusSeconds(10))
        val result = future.value.get.get.asInstanceOf[Track]
        result.waypoints.isEmpty should be (true)
      }
      "respond with all since a time" in {
        ...
        test ! Waypoint("id", now.minusSeconds(90L), Somewhere, No_Tags)
        test ! Waypoint("id", now.minusSeconds(80L), Somewhere, No_Tags)
        val future = test ? Tracker.Query("id", now.minusSeconds(95))
        val result = future.value.get.get.asInstanceOf[Track]
      ...
      "respond with all since a time by id" in {
        ...
        test ! Waypoint("id", now.minusSeconds(100L), Somewhere, No_Tags)
        test ! Waypoint("id", now.minusSeconds(90L), Somewhere, No_Tags)
        test ! Waypoint("NO_MATCH", now.minusSeconds(85L), Somewhere, No_Tags)
        test ! Waypoint("id", now.minusSeconds(80L), Somewhere, No_Tags)
        val future = test ? Tracker.Query("id", now.minusSeconds(95))
        val result = future.value.get.get.asInstanceOf[Track]
        result.waypoints.length should be (2)
